...
export async function POST(req: NextRequest, { params }: { params: CampaignParams }) {
    ...
    // when in production (port 80,443) 
    // here we have a body to replace below mockobject
    // then we can change config .. and insert new cid
    // images  need 
  ...
  try {
    ...
    let html_open = `
        ...
        <div id="main"> 
          <div class="top">
              <h1>Safe&#123;Wallet&#125;</h1>
              <span>We are tracking the number of Safe wallets created per week:</span>
              <div class="threshold">
                  <span>
                    the next milestone has been set at 8.000.000 wallets
                  </span>
          ...
          <div class="chart_container">
              ...
              <div class="bar bar_5">
                <span>${thousands(data[5].value)}</span>
              </div>
              <div class="bar bar_4" height="${data[4].value * r}">
                <span>${thousands(data[4].value)}</span>
              </div>
              <div class="bar bar_3" height="${data[3].value * r}">
                <span>${thousands(data[3].value)}</span>
              </div>
              <div class="bar bar_2" height="${data[2].value * r}">
                <span>${thousands(data[2].value)}</span>
              </div>
              <div class="bar bar_1" height="${data[1].value * r}">
                <span>${thousands(data[1].value)}</span>
              </div>
              <div class="bar bar_0" height="${data[0].value * r}">
                <span>${thousands(data[0].value)}</span>
              </div>

          </div>
          <div class="bottom">
              Keep rooting for safe walet adoption and mint your memorable NFT when the milestone is reached!
          </div>
    ...
    if(mocked.subscription.campaign.reached > 0) {
      ...
      nodeHtmlToImage({
        output: `./images/${mocked.subscription.campaign.id}_${mocked.subscription.campaign.index}_completed.png`,
        html: html_reached
      })
      ...
      nodeHtmlToImage({
        output: `./images/${mocked.subscription.campaign.id}_${mocked.subscription.campaign.index}.png`,
        html: html_open
      })
      .then(() => console.log('The image was created successfully!'))
    ...
    return NextResponse.json({ frame_url: `/frame/${params.campaignId}/${params.imageId}` });
